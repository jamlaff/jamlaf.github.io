...
2. 개념 증명 실습 - DVWA Security Level(Low, Midium, High)로 구성된 피싱사이트 유도 및 악성스크립트 실행 실습 진행
...
사용자가 웹 서버에 접속할 때 사용자가 사용하는 웹 어플리케이션은 서버에 요청을 보내게된다. 이에 서버는 사용자가 요청하는 모든 데이터를 응답할 때 쿠키값을 포함하여 응답하게 된다. 그 이후 사용자가 웹 서버에 또 다른 요청을 보낼 때 전달받은 쿠키값을 같이 포함하여 사용자라는 것을 인증하여 보내게 된다. (웹 서버마다 주고받는 쿠키값은 다르며 웹 서버가 지정해놓은 시간 동안만 유효하다)

#### - CSRF 공격 시나리오 설명

1. 사용자가 특정 웹 사이트에 로그인하여 접속한다.
2. 사용자가 로그인 후 패스워드 변경 시 웹 서버에 전달되는 HTTP 요청을 가로챈 뒤 쿠키값을 탈취한다.
3. 공격자는 본인이 만든 악성스크립트가 포함된 페이지 링크를 사용자에게 email로 전송하고 속여 공격자의 스크립트가 수행되도록 유도한다.
...
사용자가 Get Method로 패스워드 변경 요청을 진행했을 때 변경된 패스워드 정보가 URL에 표시되는 것을 확인할 수 있었다. 

![이미지](/assets/passwordchange_url2.png)

CSRF 공격을 수행하기 위해 공격자가 피싱사이트에 업로드 할 악성스크립트를 아래와 같이 작성해보았다.
...
사용자가 해당 웹 서버에 이미 로그인 되어있다고 가정하고 다른 웹 사이트에서 링크를 클릭하였을 때 `http://localhost/vulnerabilities/csrf/?password_new=hacked&password_conf=hacked&Change=Change` 이와 같은 요청이 전달된다면 사용자 검증 없이 패스워드가 변경될 수 있을 것이다. 이를 이용해 공격자는 피싱사이트를 제작하여 사용자에게 메일로 전송하고, 피싱사이트 안에 포함된 링크를 직접 클릭 할 수 있도록 유도하는 공격을 수행해 보았다.
...
사용자에게 은행 사이트의 계정 보안강화를 위해 악성스크립트를 클릭할 수 있도록 유도하였다. 일반 사용자 입장에서는 URL을 클릭 후 보안이 강화 되었다고 생각할 것이다. 그 이후 DVWA 페이지에 사용자 계정 및 패스워드를 입력했을 때 공격자의 패스워드 변경 악성스크립트로 인해 공격자가 패스워드 변조 공격에 성공한 것을 확인하였다.
