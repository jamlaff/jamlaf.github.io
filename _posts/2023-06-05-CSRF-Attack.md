...
CSRF 공격을 수행하기 위해 공격자가 피싱사이트에 업로드 할 악성 스크립트를 아래와 같이 작성해보았다.
...
사용자에게 은행 사이트의 계정 보안강화를 위해 악성 스크립트를 클릭할 수 있도록 유도하였다. 일반 사용자 입장에서는 URL을 클릭 후 보안이 강화 되었다고 생각할 것이다. 그 이후 DVWA 페이지에 사용자 계정 및 패스워드를 입력했을 때 공격자의 패스워드 변경 악성 스크립트로 인해 공격자가 패스워드 변조 공격에 성공한 것을 확인하였다.
