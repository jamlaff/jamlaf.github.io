...
2. 사용자가 로그인 시 HTTP 요청을 가로챈 뒤 쿠키값을 탈취한다.
3. 공격자는 본인이 만든 악성 스크립트가 포함된 페이지 링크를 사용자에게 email로 전송하고 사용자를 속여 공격자의 스크립트가 수행되도록 한다.
4. 사용자가 링크를 클릭할 때 공격자가 의도한대로 공격자가 지정한 패스워드로 요청을 변조하여 서버에 전달한다.
5. 서버는 쿠키 값이 담긴 sessionID를 통해 인증된 사용자로부터 온 것인지 판단하고 요청을 처리한다.
6. 공격자는 의도한대로 패스워드를 변경하고 인증에 성공한다. (사용자 계정 탈취)

#### * 쿠키와 세션
우선 CSRF 공격을 이해하기 위해서는 쿠키와 세션에 대한 이해가 필요하다. 사용자가 웹 서버에 접속하여 요청을 보낼 때 다음과 같은 작업들이 수행된다.

사용자가 웹 서버에 접속할 때 사용자가 사용하는 웹 어플리케이션은 서버에 요청을 보내게된다. 이에 서버는 사용자가 요청하는 모든 데이터를 응답할 때 쿠키 값을 포함하여 응답하게 된다. 그 이후 사용자가 웹 서버에 또 다른 요청을 보낼 때 전달받은 쿠키를 같이 포함하여 사용자라는 것을 인증하여 보내게 된다. (웹 서버마다 주고받는 쿠키값은 다르며 웹 서버가 지정해놓은 시간 동안만 유효하다)
...
3. 공격자는 서버에 인증된 브라우저의 사용자가 악성 스크립트 페이지를 누르도록 유도한다.
   - 게시판에 악성 스크립트를 게시글로 작성하여 관리자 혹은 다른 사용자들이 게시글을 클릭 하도록 유도하는 기법
   - 메일 등으로 악성 스크립트를 직접 전달하거나, 악성 스크립트가 적힌 페이지 링크를 전달하는 기법
4. 사용자가 악성 스크립트가 작성된 페이지에 접근 시 쿠키에 저장된 sessionID는 브라우저에 의해 자동적으로 함께 서버로 요청한다.
5. 서버는 쿠키가 담긴 sessionID를 통해 해당 요청이 인증된 사용자로부터 온 것으로 판단하고 처리한다.

### 개념 증명

#### * Low Level

DVWA 웹 서버에서 사용자 패스워드 변경 페이지에서 실습을 진행해보았다.

![이미지](/assets/passwordchange_url1.png)

공격자가 HTTP 요청을 가로채어 페이로드를 확인하였다.

![이미지](/assets/intercept.png)

URL에 표시되는 페이로드에 패스워드 변경 시에도 공격자가 원하는대로 변경되는 것을 볼 수 있었다.

![이미지](/assets/passwordchange_url2.png)

![이미지](/assets/csrfattackscript.png)
...
<script language="javascript">
	function attack(){
		var host = "localhost";
		var req="http://localhost/vulnerabilities/csrf/?password_new=hacked&password_conf=hacked&Change=Change"; //웹 서버에 요청을 위조하여 hacked 값으로 패스워드를 변경
		...
		alert("Thank you for your support! Your Account is mine"); //링크를 클릭했을 때의 알림창
	}
</script>
<body>
...
일반 사용자 입장에서는 URL을 클릭 후 보안이 강화 되었다고 생각할 것이다.

![이미지](/assets/로그인실패.png)

공격자의 피싱사이트에서 링크를 클릭했을 때의 요청을 Burpsuite를 통해 Proxy서버로 확인하고 이 전과 비교해보았다.

![이미지](/assets/hacked_intercept.png)

정상적인 요청과 CSRF공격을 통한 요청을 비교해 보았을 때 쿠키값이 같은 것을 비교해 보았다.
...
```
요청을 두 번 보내게 된다. DVWA에서 CSRF 메뉴를 누를 때 발생하는 요청
request 함수 내에서 user_token을 알아내고 알아낸 토큰을 두 번째 요청에 포함하여 보낸다. 우회가 가능하다.
Javascript 파일을 XSS공격이 가능한 페이지에서 스크립트 파일을 업로드 하고
